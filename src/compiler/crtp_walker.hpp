...
private:
  ...
  auto conv_expression(const ast::Expr& expr, auto... args) {
    return (static_cast<Derived*>(this))->template expression<T>(dynamic_cast<const T&>(expr), args...);
  }
  ...
  void conv_statement(const ast::Stmt& stat, auto... args) {
    return (static_cast<Derived*>(this))->template statement<T>(dynamic_cast<const T&>(stat), args...);
  }
};
} // namespace yume
